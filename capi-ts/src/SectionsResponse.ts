...
export interface ISectionsResponse {
  ...
  userTier: string;
  total: number;
  results: Array<__NAMESPACE__.ISection>;
}
export interface ISectionsResponseArgs {
  status: string;
  userTier: string;
  total: number;
  results: Array<__NAMESPACE__.ISectionArgs>;
}
export const SectionsResponseCodec: thrift.IStructCodec<
  ISectionsResponseArgs,
  ISectionsResponse
> = {
  encode(args: ISectionsResponseArgs, output: thrift.TProtocol): void {
    ...
    if (obj.results != null) {
      ...
      obj.results.forEach(
        (value_1: __NAMESPACE__.ISectionArgs): void => {
          __NAMESPACE__.SectionCodec.encode(value_1, output);
        }
      );
      output.writeListEnd();
      output.writeFieldEnd();
  ...
  decode(input: thrift.TProtocol): ISectionsResponse {
      ...
      switch (fieldId) {
        ...
        case 4:
          if (fieldType === thrift.TType.LIST) {
            const value_5: Array<__NAMESPACE__.ISection> = new Array<
              __NAMESPACE__.ISection
            >();
    ...
    if (
      ...
      throw new thrift.TProtocolException(
        thrift.TProtocolExceptionType.UNKNOWN,
        'Unable to read SectionsResponse from input'
      );
    }
  },
};
export class SectionsResponse extends thrift.StructLike
  implements ISectionsResponse {
  public status: string;
  public userTier: string;
  public total: number;
  public results: Array<__NAMESPACE__.ISection>;
  ...
  constructor(args: ISectionsResponseArgs) {
    ...
    if (args.results != null) {
      const value_10: Array<__NAMESPACE__.ISection> = new Array<
        __NAMESPACE__.ISection
      >();
      args.results.forEach(
        (value_11: __NAMESPACE__.ISectionArgs): void => {
          const value_12: __NAMESPACE__.ISection = new __NAMESPACE__.Section(
            value_11
          );
