...
export interface IIngredientsList {
  title?: string;
  ingredients: Array<__NAMESPACE__.IIngredient>;
}
export interface IIngredientsListArgs {
  title?: string;
  ingredients: Array<__NAMESPACE__.IIngredientArgs>;
}
export const IngredientsListCodec: thrift.IStructCodec<
  IIngredientsListArgs,
  IIngredientsList
> = {
  encode(args: IIngredientsListArgs, output: thrift.TProtocol): void {
    ...
    if (obj.ingredients != null) {
      ...
      obj.ingredients.forEach(
        (value_1: __NAMESPACE__.IIngredientArgs): void => {
          __NAMESPACE__.IngredientCodec.encode(value_1, output);
        }
      );
      output.writeListEnd();
      output.writeFieldEnd();
      ...
      throw new thrift.TProtocolException(
        ...
        'Required field[ingredients] is unset!'
      );
    }
  ...
  decode(input: thrift.TProtocol): IIngredientsList {
      ...
      switch (fieldId) {
        ...
        case 2:
          if (fieldType === thrift.TType.LIST) {
            const value_3: Array<__NAMESPACE__.IIngredient> = new Array<
              __NAMESPACE__.IIngredient
            >();
            const metadata_1: thrift.IThriftList = input.readListBegin();
            const size_1: number = metadata_1.size;
            for (let i_1: number = 0; i_1 < size_1; i_1++) {
              const value_4: __NAMESPACE__.IIngredient = __NAMESPACE__.IngredientCodec.decode(
                input
              );
    ...
    if (_args.ingredients !== undefined) {
      ...
      throw new thrift.TProtocolException(
        thrift.TProtocolExceptionType.UNKNOWN,
        'Unable to read IngredientsList from input'
      );
    }
  },
};
export class IngredientsList extends thrift.StructLike
  implements IIngredientsList {
  public title?: string;
  public ingredients: Array<__NAMESPACE__.IIngredient>;
  ...
  constructor(args: IIngredientsListArgs) {
    ...
    if (args.ingredients != null) {
      const value_6: Array<__NAMESPACE__.IIngredient> = new Array<
        __NAMESPACE__.IIngredient
      >();
      args.ingredients.forEach(
        (value_7: __NAMESPACE__.IIngredientArgs): void => {
          const value_8: __NAMESPACE__.IIngredient = new __NAMESPACE__.Ingredient(
            value_7
          );
