  ...
  encode(args: IQuestionArgs, output: thrift.TProtocol): void {
    ...
    if (obj.questionId != null) {
      ...
      throw new thrift.TProtocolException(
        ...
        'Required field[questionId] is unset!'
      );
    }
    ...
    if (obj.answers != null) {
      ...
      obj.answers.forEach(
        (value_1: __NAMESPACE__.IAnswerArgs): void => {
      ...
      switch (fieldId) {
        ...
        case 3:
          if (fieldType === thrift.TType.LIST) {
            const value_4: Array<__NAMESPACE__.IAnswer> = new Array<
              __NAMESPACE__.IAnswer
            >();
            const metadata_1: thrift.IThriftList = input.readListBegin();
            const size_1: number = metadata_1.size;
            for (let i_1: number = 0; i_1 < size_1; i_1++) {
              const value_5: __NAMESPACE__.IAnswer = __NAMESPACE__.AnswerCodec.decode(
                input
              );
...
export class Question extends thrift.StructLike implements IQuestion {
  ...
  constructor(args: IQuestionArgs) {
    ...
    if (args.answers != null) {
      const value_8: Array<__NAMESPACE__.IAnswer> = new Array<
        __NAMESPACE__.IAnswer
      >();
      args.answers.forEach(
        (value_9: __NAMESPACE__.IAnswerArgs): void => {
          const value_10: __NAMESPACE__.IAnswer = new __NAMESPACE__.Answer(
            value_9
          );
