...
export const QuestionCodec: thrift.IStructCodec<IQuestionArgs, IQuestion> = {
    encode(args: IQuestionArgs, output: thrift.TProtocol): void {
        ...
        if (obj.answers != null) {
            ...
            obj.answers.forEach((value_1: Answer.IAnswerArgs): void => {
                Answer.AnswerCodec.encode(value_1, output);
            });
            output.writeListEnd();
            output.writeFieldEnd();
        }
            ...
            switch (fieldId) {
                ...
                case 3:
                    if (fieldType === thrift.TType.LIST) {
                        const value_4: Array<Answer.IAnswer> = new Array<Answer.IAnswer>();
                        const metadata_1: thrift.IThriftList = input.readListBegin();
                        const size_1: number = metadata_1.size;
                        for (let i_1: number = 0; i_1 < size_1; i_1++) {
                            const value_5: Answer.IAnswer = Answer.AnswerCodec.decode(input);
...
export class Question extends thrift.StructLike implements IQuestion {
    ...
    constructor(args: IQuestionArgs) {
        ...
        if (args.answers != null) {
            const value_8: Array<Answer.IAnswer> = new Array<Answer.IAnswer>();
            args.answers.forEach((value_9: Answer.IAnswerArgs): void => {
                const value_10: Answer.IAnswer = new Answer.Answer(value_9);
