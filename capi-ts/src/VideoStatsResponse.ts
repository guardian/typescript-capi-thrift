...
export const VideoStatsResponseCodec: thrift.IStructCodec<
  IVideoStatsResponseArgs,
  IVideoStatsResponse
> = {
  encode(args: IVideoStatsResponseArgs, output: thrift.TProtocol): void {
    ...
    if (obj.mostViewedVideos != null) {
      ...
      obj.mostViewedVideos.forEach(
        (value_1: __NAMESPACE__.IMostViewedVideoArgs): void => {
      ...
      throw new thrift.TProtocolException(
        thrift.TProtocolExceptionType.UNKNOWN,
        'Required field[mostViewedVideos] is unset!'
      );
    }
  ...
  decode(input: thrift.TProtocol): IVideoStatsResponse {
      ...
      switch (fieldId) {
        ...
        case 2:
          if (fieldType === thrift.TType.LIST) {
            const value_3: Array<__NAMESPACE__.IMostViewedVideo> = new Array<
              __NAMESPACE__.IMostViewedVideo
            >();
            const metadata_1: thrift.IThriftList = input.readListBegin();
            const size_1: number = metadata_1.size;
            for (let i_1: number = 0; i_1 < size_1; i_1++) {
              const value_4: __NAMESPACE__.IMostViewedVideo = __NAMESPACE__.MostViewedVideoCodec.decode(
                input
              );
    ...
    if (_args.status !== undefined && _args.mostViewedVideos !== undefined) {
      ...
      throw new thrift.TProtocolException(
        thrift.TProtocolExceptionType.UNKNOWN,
        'Unable to read VideoStatsResponse from input'
      );
    }
  },
};
export class VideoStatsResponse extends thrift.StructLike
  implements IVideoStatsResponse {
  ...
  constructor(args: IVideoStatsResponseArgs) {
    ...
    if (args.mostViewedVideos != null) {
      const value_6: Array<__NAMESPACE__.IMostViewedVideo> = new Array<
        __NAMESPACE__.IMostViewedVideo
      >();
      args.mostViewedVideos.forEach(
        (value_7: __NAMESPACE__.IMostViewedVideoArgs): void => {
          const value_8: __NAMESPACE__.IMostViewedVideo = new __NAMESPACE__.MostViewedVideo(
            value_7
          );
