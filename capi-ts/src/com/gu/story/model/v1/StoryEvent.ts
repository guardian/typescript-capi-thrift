...
export interface IStoryEvent {
  ...
  name: string;
  summary: string;
  content: Array<__NAMESPACE__.IContent>;
}
export interface IStoryEventArgs {
  id: string;
  name: string;
  summary: string;
  content: Array<__NAMESPACE__.IContentArgs>;
}
export const StoryEventCodec: thrift.IStructCodec<
  IStoryEventArgs,
  IStoryEvent
> = {
  ...
  decode(input: thrift.TProtocol): IStoryEvent {
      ...
      switch (fieldId) {
        ...
        case 4:
          if (fieldType === thrift.TType.LIST) {
            const value_5: Array<__NAMESPACE__.IContent> = new Array<
              __NAMESPACE__.IContent
            >();
            const metadata_1: thrift.IThriftList = input.readListBegin();
            const size_1: number = metadata_1.size;
            for (let i_1: number = 0; i_1 < size_1; i_1++) {
              const value_6: __NAMESPACE__.IContent = __NAMESPACE__.ContentCodec.decode(
                input
              );
    ...
    if (
      ...
      throw new thrift.TProtocolException(
        thrift.TProtocolExceptionType.UNKNOWN,
        'Unable to read StoryEvent from input'
      );
    }
  },
};
export class StoryEvent extends thrift.StructLike implements IStoryEvent {
  ...
  public content: Array<__NAMESPACE__.IContent>;
  ...
  constructor(args: IStoryEventArgs) {
    ...
    if (args.content != null) {
      const value_10: Array<__NAMESPACE__.IContent> = new Array<
        __NAMESPACE__.IContent
      >();
      args.content.forEach(
        (value_11: __NAMESPACE__.IContentArgs): void => {
          const value_12: __NAMESPACE__.IContent = new __NAMESPACE__.Content(
            value_11
          );
