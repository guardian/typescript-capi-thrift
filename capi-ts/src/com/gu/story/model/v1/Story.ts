...
import * as StoryEvent from "./StoryEvent";
import * as com_gu_contentatom_thrift from "./../../../contentatom/thrift";
export interface IStory {
    id: string;
    name: string;
    summary: string;
    events: Array<StoryEvent.IStoryEvent>;
...
export interface IStoryArgs {
    ...
    name: string;
    summary: string;
    events: Array<StoryEvent.IStoryEventArgs>;
...
export const StoryCodec: thrift.IStructCodec<IStoryArgs, IStory> = {
    encode(args: IStoryArgs, output: thrift.TProtocol): void {
        ...
        if (obj.events != null) {
            ...
            obj.events.forEach((value_1: StoryEvent.IStoryEventArgs): void => {
                StoryEvent.StoryEventCodec.encode(value_1, output);
            });
            output.writeListEnd();
            output.writeFieldEnd();
        }
            ...
            switch (fieldId) {
                ...
                case 4:
                    if (fieldType === thrift.TType.LIST) {
                        const value_6: Array<StoryEvent.IStoryEvent> = new Array<StoryEvent.IStoryEvent>();
                        const metadata_1: thrift.IThriftList = input.readListBegin();
                        const size_1: number = metadata_1.size;
                        for (let i_1: number = 0; i_1 < size_1; i_1++) {
                            const value_7: StoryEvent.IStoryEvent = StoryEvent.StoryEventCodec.decode(input);
...
export class Story extends thrift.StructLike implements IStory {
    ...
    public events: Array<StoryEvent.IStoryEvent>;
    ...
    constructor(args: IStoryArgs) {
        ...
        if (args.events != null) {
            const value_13: Array<StoryEvent.IStoryEvent> = new Array<StoryEvent.IStoryEvent>();
            args.events.forEach((value_15: StoryEvent.IStoryEventArgs): void => {
                const value_16: StoryEvent.IStoryEvent = new StoryEvent.StoryEvent(value_15);
