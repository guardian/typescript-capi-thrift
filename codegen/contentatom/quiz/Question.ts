...
export const QuestionCodec: thrift.IStructCodec<IQuestionArgs, IQuestion> = {
    encode(args: IQuestionArgs, output: thrift.TProtocol): void {
        ...
        if (obj.answers != null) {
            ...
            obj.answers.forEach((value_2: __NAMESPACE__.IAnswerArgs): void => {
                __NAMESPACE__.AnswerCodec.encode(value_2, output);
            });
            output.writeListEnd();
            output.writeFieldEnd();
        }
            ...
            switch (fieldId) {
                ...
                case 2:
                    if (fieldType === thrift.TType.LIST) {
                        const value_4: Array<__NAMESPACE__.IAsset> = new Array<__NAMESPACE__.IAsset>();
                        const metadata_1: thrift.IThriftList = input.readListBegin();
                        const size_1: number = metadata_1.size;
                        for (let i_1: number = 0; i_1 < size_1; i_1++) {
                            const value_5: __NAMESPACE__.IAsset = __NAMESPACE__.AssetCodec.decode(input);
                ...
                case 3:
                    if (fieldType === thrift.TType.LIST) {
                        const value_6: Array<__NAMESPACE__.IAnswer> = new Array<__NAMESPACE__.IAnswer>();
                        const metadata_2: thrift.IThriftList = input.readListBegin();
                        const size_2: number = metadata_2.size;
                        for (let i_2: number = 0; i_2 < size_2; i_2++) {
                            const value_7: __NAMESPACE__.IAnswer = __NAMESPACE__.AnswerCodec.decode(input);
...
export class Question extends thrift.StructLike implements IQuestion {
    ...
    constructor(args: IQuestionArgs) {
        ...
        if (args.assets != null) {
            const value_10: Array<__NAMESPACE__.IAsset> = new Array<__NAMESPACE__.IAsset>();
            args.assets.forEach((value_13: __NAMESPACE__.IAssetArgs): void => {
                const value_14: __NAMESPACE__.IAsset = new __NAMESPACE__.Asset(value_13);
        ...
        if (args.answers != null) {
            const value_11: Array<__NAMESPACE__.IAnswer> = new Array<__NAMESPACE__.IAnswer>();
            args.answers.forEach((value_15: __NAMESPACE__.IAnswerArgs): void => {
                const value_16: __NAMESPACE__.IAnswer = new __NAMESPACE__.Answer(value_15);
