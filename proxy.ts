...
import { ItemResponseCodec } from "./codegen";

const app = express()
const port = 3131
const url = path => `https://content.guardianapis.com/${path}?format=thrift&api-key=${process.env.capikey}&show-elements=all&show-atoms=all&show-rights=all&show-fields=all&show-tags=all&show-blocks=all&show-references=all`
...
const getArticle = async (path: string) => {
    ...
    const title = data.content.webTitle
    const body = data.content.blocks.body.map(_ => _.elements)
    return [title, body]
